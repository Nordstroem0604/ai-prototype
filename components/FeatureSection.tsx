...
import SidebarFeatureButton from '@/components/SidebarFeatureButton'; // husk at du har oprettet denne i sin egen fil
  ...
  return (
    <div className={`transition-all duration-300 ${isFeatureBoxVisible ? 'w-full' : 'w-0 overflow-hidden'}`}>
      <div className={`${bgColor} rounded-lg shadow-lg border ${borderColor}`}>
        <div className="flex">
          {/* Sidebar */}
          <div className={`border-r ${borderColor} transition-all duration-300 
          ...
          <div className="flex-1 p-6">
            ...
            <div className="space-y-4">
              ...
              <div className={`mt-8 p-4 rounded-lg ${isDark ? 'bg-slate-700/50' : 'bg-slate-50'}`}>
                <h3 className={`${textColor} font-medium mb-4`}>Additional Information</h3>
                <p className={`${isDark ? 'text-slate-300' : 'text-slate-600'} text-sm`}>
                  Explore more details about this feature and its capabilities.
                </p>
              </div>
            </div>
          </div>
