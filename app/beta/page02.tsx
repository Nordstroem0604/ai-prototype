...
const CombinedLayout = () => {
  ...
  const FeatureButton = ({ feature }: { feature: FeatureContent }) => (
    <button
      ...
      className={`w-full text-left font-medium p-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-500 ${
      ...
      {feature.id === 'overview' ? (
          ...
          {features.filter(f => f.id !== 'overview').map((f) => (
            <div key={f.id} 
                 className={`p-4 rounded-lg border cursor-pointer ${
                   isDark ? 'bg-neutral-600 border-neutral-800 hover:border-gray-500' : 'bg-white border-gray-300 hover:border-indigo-500'
                 }`}
        ...
        <div className="flex space-x-4">
          <div className={`w-1/2 p-4 rounded-lg border ${
            ...
            <img src="https://placehold.co/150x150" alt="feature preview" className="rounded-lg mb-2" />
            <p className={`text-sm ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>
              {feature.description}
            </p>
          </div>
          <div className={`w-1/2 p-4 rounded-lg border ${
            ...
            <img src="https://placehold.co/150x150" alt="feature details" className="rounded-lg mb-2" />
            <p className={`text-sm ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>
  ...
  return (
    <div className={`min-h-screen ${isDark ? 'bg-neutral-800' : 'bg-gray-100'} transition-colors duration-300`}>
      {/* Header */}
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            className={`p-2 rounded-full hover:bg-opacity-10 hover:bg-gray-500 ${
              showFeatures ? (isDark ? 'bg-gray-700' : 'bg-gray-200') : ''
            }`}
      ...
      <div className="container mx-auto px-6 flex gap-6">
        ...
        {showFeatures && (
          <div className="w-2/3">
            <div className={`rounded-lg shadow-lg ${isDark ? 'bg-neutral-600' : 'bg-white'}`}>
              <div className="flex">
                {/* Sidebar */}
                <div className={`w-1/4 p-6 border-r ${
                  isDark ? 'bg-neutral-700 border-neutral-800' : 'bg-gray-50 border-gray-200'
                }`}>
